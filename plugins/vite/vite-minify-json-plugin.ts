...
export function minifyPublicJsonFiles(externalAssets = false): VitePlugin {
  let logger: Logger;
  let count = 0;
  const errors: Error[] = [];
  ...
  return {
    ...
    async generateBundle(options): Promise<void> {
      ...
      // The sandbox deployment serves upstream game media through its worker.
      // Keeping these large, unchanged files out of the application bundle makes
      // the client small enough for the hosting platform while preserving paths.
      if (externalAssets) {
        logger.info(cyan("Skipping bundled game media for external asset delivery."));
        return;
      }
