...
// A new key intentionally ignores the previous experimental bot's saved ON state.
// The bot always starts OFF after this rollout and only acts after an explicit click.
const BOT_ENABLED_KEY = "pokerogue-sandbox-autoplay-enabled-v2";
...
export class AutoplayManager {
  ...
  private lastAction = "Bot je vypnutý";
  private lastMode = -1;
  ...
  constructor() {
    this.enabled = window.localStorage.getItem(BOT_ENABLED_KEY) === "true";
    this.createControls();
    this.exposeTestApi();
    this.updateControls();
  }
